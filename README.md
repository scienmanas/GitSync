...
2. **Install dependencies (because Python loves packages):**
   ...
   This creates `.venv/` and installs everything pinned in `uv.lock`. Requires Python 3.12+. A `requirements.txt` is also kept in sync (regenerated by `build.sh`) for any tooling that still needs it.
